...
// Reporte detallado de una campaña específica
...
// Reporte de rendimiento general del usuario
...
const getSystemReport = async (req, res) => {
  try {
    // Verificar que es administrador (ya validado por middleware)
...
// Reporte de usuarios para administradores
const getUsersReport = async (req, res) => {
  try {
    ...
    const userStats = await Promise.all(users.map(async (user) => {
      ...
      const balance = await require('../models/balanceModel').Balance.findOne({ userId: user._id });
